...
//				reactHtml = (<Input onPressEnter={this.searchClick} placeholder={`请输入${val.title}`} disabled={loading}/>)
  ...
  render() {
	let { search,onSearchReset,loading,resetFresh } = this.props;
	search = search.filter((s,i)=>s && Object.keys(s).length > 0);//去除空的对象或不存在的东西
    return (
