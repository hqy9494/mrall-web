...
export default class TableExpand extends React.Component {
  ...
  forSearchList = () =>{
		const { hasInitValue,pathParam } = this.props;
  	if(!hasInitValue)return;
  	const params = getParameterByName(pathParam || 'q')? JSON.parse(getParameterByName(pathParam || 'q')) : {};
  	let { searchs=[] } = params;
  ...
  getData = (cb, props,isRefresh) => {
		const { api,pathParam,refreshSkin } = props || this.props;
		this.params = getParameterByName(pathParam || 'q')? JSON.parse(getParameterByName(pathParam || 'q')) : {};
  ...
  dealColumns = columns => {
	...
	//更改头部地址栏信息（无跳转）
  jumpUrl = (newParams = {},cb) => {

  	const { path,pathParam } = this.props;
    if(newParams.searchs){
    	...
    	history.replaceState(null,null,'#'+path+`?${pathParam || "q"}=${encodeURIComponent(JSON.stringify(params))}`);
    	this.forSearchList()
    	return;
    }
    ...
    const params = Object.assign({}, this.params, newParams);
    const paramsRturn = this.jdHasParams();
		const toStringParams = `?${pathParam || "q"}=${encodeURIComponent(JSON.stringify(params))}`;
    if (paramsRturn) {
...
formatValue = (type, value) => {
	...
	returnRender = (columns,search)=>{
		const { hasInitValue,api,uuid, buttons, pages ,isTabs,defaultKey,TabsData,path,expandedRowRender,resetFresh,getArea,pathParam,tableKey } = this.props;
